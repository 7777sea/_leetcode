 ...
 * @param type
 */
const checkDictDatabase = (type) => {
    switch (type) {
        case "pinyin_dict_firstletter":
            throw new Error("请先加载pinyin_dict_firstletter字典库")
        case "pinyin_dict_notone":
            throw new Error("请先加载pinyin_dict_notone字典库")
        case "pinyin_dict_withtone":
            throw new Error("请先加载pinyin_dict_withtone字典库")
        case "pinyin_dict_polyphone":
            throw new Error("请先加载pinyin_dict_polyphone字典库")
    }
};
...
const pinyinUtil = {
    ...
    parseDict: function () {
        ...
        if (dictDatabase.pinyin_dict_notone) {
            dict.notone = {};
            dict.py2hz = dictDatabase.pinyin_dict_notone; // 拼音转汉字
            for (var i in dictDatabase.pinyin_dict_notone) {
                var temp = dictDatabase.pinyin_dict_notone[i];
        ...
        if (dictDatabase.pinyin_dict_withtone) {
            dict.withtone = {}; // 汉字与拼音映射，多音字用空格分开，类似这种结构：{'大': 'da tai'}
            var temp = dictDatabase.pinyin_dict_withtone.split(',');
            ...
            if (dictDatabase.pinyin_dict_notone) {
                ...
                var notone = pinyinUtil.removeTone(dictDatabase.pinyin_dict_withtone).split(',');
                var py2hz = {}, py, hz;
    ...
    getPinyin: function (chinese, splitter, withtone, polyphone) {
        ...
        if (!polyphone) return result.join(splitter);
            ...
            checkDictDatabase("pinyin_dict_polyphone");
            if (dictDatabase.pinyin_dict_polyphone) return parsePolyphone(chinese, result, splitter, withtone);
...
function parsePolyphone(chinese, result, splitter, withtone) {
    ...
    var poly = dictDatabase.pinyin_dict_polyphone;
